...
{
	public LedgerQuarter importQuarter(Path workbookPath, String sheetName,
		ChartTranslationMap translation) throws IOException
	{
			...
			Workbook wb = WorkbookFactory.create(in))
		{
			...
			if (headerRowIdx < 0)
				throw new IOException("Could not locate header row");
			...
			for (int r = startDataRow; r <= lastRow; r++)
			{
				...
				row.setCheckNumber(
					CellUtil.readString(poiRow, cols.colCheckNum));
				row.setClearedBankTag(
					CellUtil.readString(poiRow, cols.colClearBank));
				row.setToFrom(CellUtil.readString(poiRow, cols.colToFrom));
				row.setMemo(CellUtil.readString(poiRow, cols.colMemo));
				row.setBudgetNotes(
					CellUtil.readString(poiRow, cols.colBudgetNotes));
	...
	private ColumnIndex mapColumns(Row headerRow)
	{
		...
		idx.group2 = detectSplitGroup(headerRow,
			(idx.group1 != null ? idx.group1.amountCol + 1 : 0));
		idx.group3 = detectSplitGroup(headerRow,
			(idx.group2 != null ? idx.group2.amountCol + 1 : 0));
		idx.group4 = detectSplitGroup(headerRow,
	...
	private SplitGroup detectSplitGroup(Row headerRow, int startSearchCol)
	{
		...
		for (int c = startSearchCol; c < maxCol; c++)
		{
			...
			if (h0 == null || !h0.trim().equalsIgnoreCase("AMOUNT"))
				continue;
			...
			if (h1 == null || h2 == null || h3 == null || h4 == null)
				continue;
			String u1 = h1.trim().toUpperCase(), u2 = h2.trim().toUpperCase(),
				u3 = h3.trim().toUpperCase(), u4 = h4.trim().toUpperCase();
			boolean looksRight = u1.startsWith("ASSET/LIABILITY ACCOUNT") &&
	...
	private void addSplitFromGroup(Row poiRow, SplitGroup g, LedgerRow out,
		ChartTranslationMap tx)
	{
		if (g == null)
			return;
	...
	{
		int colDate = -1, colCheckNum = -1, colClearBank = -1, colToFrom = -1,
			colMemo = -1, colBudgetNotes = -1;
